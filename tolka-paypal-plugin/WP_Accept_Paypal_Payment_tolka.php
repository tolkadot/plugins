<?php
/*
Plugin Name: WP Easy Paypal Payment Accept - Tolka
Version: v4.9.2
...
function wp_pp_plugin_install() {
    ...
    add_option('payment_button_type', 'https://www.paypalobjects.com/webstatic/en_US/i/btn/png/silver-rect-paypal-34px.png');
...
function Paypal_payment_accept() {
    ...
    $itemName7 = get_option('wp_pp_payment_item7');
    $value7 = get_option('wp_pp_payment_value7');
    ...
    $output .= '<option value="select option">select option</option>'; //set the default to be 'select option as the defualt'
    if (!empty($value1)) {
    ...
    if (!empty($value7)) {
        $output .= '<option value="'.esc_attr($value7).'">'.esc_attr($itemName7).'</option>';
    }
